...
const EquipmentDriver = ({ driver: propDriver, onDriverUpdate }) => {
...
const [loading, setLoading] = useState(false);
  ...
  const addEquipment = async (e) => {
    e.preventDefault();
    const selectedEquipment = allEquipments.find(equipment => equipment.name === newEquipment.item && equipment.type.toLowerCase() === newEquipment.type.toLowerCase());
    const costPerUnit = selectedEquipment ? selectedEquipment.costPerUnit : 0;
    const totalCost = costPerUnit * parseInt(newEquipment.quantity, 10);

    try {
      const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/drivers/${driver._id}/equipment`, {
        method: 'POST',
        headers: {
      ...
      setIsFormOpen(false); // Add this line to close the form
    } catch (error) {
  ...
  useEffect(() => {
    setLoading(true); 
    fetchEquipments();
  }, [driver._id]); // Dependency array ensures fetch is called when driver._id changes
  ...
  const deleteEquipment = async () => {
    ...
    try {
      const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/drivers/${driver._id}/equipment/${equipmentToDelete.id}?type=${equipmentToDelete.type}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` },
      });
...
<FormControl fullWidth sx={{ width: '250px',marginRight:'10px' }}>
  ...
  >
{allEquipments
  .filter(equipment => equipment.type.toLowerCase() === newEquipment.type.toLowerCase())
  .map((equipment, index) => (
